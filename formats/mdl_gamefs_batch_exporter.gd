## Batch MDL export from indexed GameFS install resources.
class_name MdlGamefsBatchExporter

const MdlModelMetadataHelper := preload("../editor/tools/mdl_model_metadata_helper.gd")


## Export indexed `.mdl` resources (and paired `.mdx` when present) to a flat output directory.
static func batch_install(
	...
	var entries: Array = gamefs.list_core_resources(query, "mdl", source_filter, limit)
	...
	for raw_entry in entries:
		...
		var mdl_path := output_dir.path_join("%s.mdl" % resref)
		if skip_existing and FileAccess.file_exists(mdl_path):
			skipped.append({
				"resref": resref,
				"mdl_path": mdl_path,
				"reason": "exists",
		...
		var mdl_bytes := _load_entry_bytes(gamefs, entry)
		if mdl_bytes.is_empty():
			failed.append({
				"resref": resref,
				"mdl_path": mdl_path,
				"message": "Failed to load MDL bytes.",
			})
			continue

		var mdx_path := output_dir.path_join("%s.mdx" % resref)
		var mdx_bytes := _load_paired_mdx_bytes(gamefs, resref)
		var wrote_mdx := false

		if not dry_run:
			var mdl_error := _write_bytes(mdl_path, mdl_bytes)
			if mdl_error != OK:
				failed.append({
					"resref": resref,
					"mdl_path": mdl_path,
			...
			if not mdx_bytes.is_empty():
				var mdx_error := _write_bytes(mdx_path, mdx_bytes)
				if mdx_error != OK:
					failed.append({
						"resref": resref,
						"mdl_path": mdl_path,
		...
		var record := {
			...
			"mdl_path": mdl_path,
			"mdx_path": mdx_path if wrote_mdx or (dry_run and not mdx_bytes.is_empty()) else "",
...
static func format_report(result: Dictionary) -> String:
	...
	lines.append(str(result.get("summary", "Install batch MDL export finished.")))
	...
	for raw_record in generated:
		...
		var line := "%s → %s" % [
			...
			str(record.get("mdl_path", "")).get_file(),
		]
		if bool(record.get("has_mdx", false)):
			line += " (+MDX)"
...
static func _load_paired_mdx_bytes(gamefs: RefCounted, resref: String) -> PackedByteArray:
	if not gamefs.has_method("resolve_resource"):
...
static func _format_summary(generated_count: int, skipped_count: int, failed_count: int) -> String:
	return "Install batch MDL export: %d exported, %d skipped, %d failed." % [
		generated_count,
		skipped_count,
		failed_count,
	]
