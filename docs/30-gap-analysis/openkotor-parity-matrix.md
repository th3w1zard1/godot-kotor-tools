...
| Texture/media editing | tpc/tga/dds/wav/lip/mdl surfaces | Partial | Q27 (shipped 2026-05-29): native SSF V1.1 parse/write + 28-slot workspace editor; TPC metadata/preview + PyKotor `texture-convert` TGA export; WAV metadata + PyKotor `sound-convert` (SFX/VO); workspace tabs + dock routing. Q28 (shipped 2026-05-29): native LIP V1.0 parse/write + LIP Sync Editor (duration, keyframe time/shape list, save/install); pipeline `lip` serialize. Waveform/audio scrub, batch LIP, and full TPC write-back remain backlog (Q29+). |
...
5. Media/tooling parity wave (LIP waveform/audio, TPC write-back, batch tooling).
6. Module/area designer parity wave (native indoor build port, 3D rotate gizmo, LYT/walkmesh depth).
