...
| Patching/diff tooling parity | HoloPatcher, KotorDiff | Partial | Q32 (shipped 2026-06-04): semantic GFF-family compare reports in install diff (`GFFCompare`); 2DA/TLK semantic compare pre-existing. Q35 (shipped 2026-06-04): `SSFCompare` slot-level StrRef diff for `.ssf` install compare. Q36 (shipped 2026-06-04): `LIPCompare` duration/keyframe diff for `.lip` install compare. Full KotorDiff UI / HoloPatcher remain backlog. |
...
12. Module/area designer parity wave (native indoor build port, 3D rotate gizmo, LYT/walkmesh depth).
