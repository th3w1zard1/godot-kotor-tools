...
| Advanced utility tools | PyKotor tools (modulekit, references, texture batch, model helpers) | Partial | Q85 (shipped 2026-06-07): install-scoped GameFS TPC batch export (`TpcGamefsBatchExporter`, override-index scan, PyKotor `texture-convert` bridge). Modulekit, folder batch import/export, and full-install reference tracing remain on separate slices/PRs. |
