...
| Advanced utility tools | PyKotor tools (modulekit, references, texture batch, model helpers) | Partial | Q83 (shipped 2026-06-07): folder batch TPC→TGA export (`TpcBatchExporter`, PyKotor `texture-convert` bridge, TPC editor batch action). Modulekit, references finder, install-scoped batch export/import remain on separate slices/PRs. |
