...
| Texture/media editing | tpc/tga/dds/wav/lip/mdl surfaces | Partial | Q27 (shipped 2026-05-29): native SSF V1.1 parse/write + 28-slot workspace editor; TPC metadata/preview + PyKotor `texture-convert` TGA export; shared `WavMetadata` + PyKotor `sound-convert` (SFX/VO); workspace tabs + dock routing. Q28 (shipped 2026-05-29): native LIP V1.0 parse/write + LIP Sync Editor (duration, keyframe time/shape list, save/install); pipeline `lip` serialize. Q29 (shipped 2026-05-29): LIP paired WAV load, PCM waveform + playhead, playback/scrub, live viseme preview, optional duration sync; `LipWaveformView`. Q31 (shipped 2026-06-04): `LipBatchGenerator` batch placeholder LIP from PCM WAV folder; LIP editor **Batch Generate LIP...**. Full TPC write-back remains backlog. |
...
6. **Q31 (shipped 2026-06-04):** Batch LIP generator — `LipBatchGenerator`, folder scan, LIP editor toolbar action; see `docs/plans/2026-06-04-004-feat-q31-batch-lip-generator-plan.md`.
7. Media/tooling parity wave (TPC write-back).
8. Module/area designer parity wave (native indoor build port, 3D rotate gizmo, LYT/walkmesh depth).
