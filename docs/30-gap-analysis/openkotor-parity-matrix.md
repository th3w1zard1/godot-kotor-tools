...
| Advanced utility tools | PyKotor tools (modulekit, references, texture batch, model helpers) | Partial | Q80 (shipped 2026-06-07): override-scoped ResRef references finder in resource browser (`KotorResRefReferenceScanner`, GFF field-path + NSS text scan, formatted detail report). Modulekit, texture batch, and full-install reference tracing remain backlog. |
