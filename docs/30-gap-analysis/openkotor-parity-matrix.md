...
| Advanced utility tools | PyKotor tools (modulekit, references, texture batch, model helpers) | Partial | Q82 (shipped 2026-06-07): folder batch TGA/PNG→TPC import (`TpcBatchConverter`, native RGBA encode, TPC editor batch action). Modulekit, references finder, and install-scoped batch export remain on separate slices/PRs. |
...
7. Media/tooling parity wave (DXT TPC encode, batch LIP tooling).
