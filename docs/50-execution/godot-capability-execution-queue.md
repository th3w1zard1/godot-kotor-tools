...
| --- | --- | --- | --- | --- |
| P1 | OpenKotOR parity program (PyKotor/Holocron) | Drive upstream parity in bounded Godot editor slices with matrix-driven backlog. | Q31 shipped batch LIP loop, parity master plan active, per-slice verification retained. | Next: TPC write-back, native indoor build, DLG depth, or diff tooling per master plan Phase C–E. |
