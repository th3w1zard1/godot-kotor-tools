...
func _initialize() -> void:
	_install_root = ProjectSettings.globalize_path("user://module_designer_pth_point_drag_test")
	DirAccess.make_dir_recursive_absolute(_install_root.path_join("override"))
	call_deferred("_run_tests")


func _run_tests() -> void:
	await _test_path_point_drag_and_install()
	_cleanup()
	print("✓ Module designer PTH point drag tests passed")
	quit()


func _test_path_point_drag_and_install() -> void:
	...
	editor._map_view.path_point_drag_finished.emit(1, 45.0, -30.0, 52.5, -12.0)
	await process_frame

	assert(editor.is_document_dirty())
	assert(editor._map_view._selected_path_point_index == 1)
	assert(editor._viewport_3d._selected_path_point_index == 1)
	assert(editor._detail_label.text.find("Point ID: 2") >= 0)
	assert(editor._detail_label.text.find("Position: 52.50, -12.00, 1.00") >= 0)

	var moved_point := editor._path_resource.get_point_records()[1]
	assert(is_equal_approx(float(moved_point.get("x", 0.0)), 52.5))
	assert(is_equal_approx(float(moved_point.get("y", 0.0)), -12.0))

	var moved_connection := editor._path_resource.get_connection_records()[1]
	assert(is_equal_approx(float(moved_connection.get("source_x", 0.0)), 52.5))
	assert(is_equal_approx(float(moved_connection.get("source_y", 0.0)), -12.0))
	print("✓ PTH point drag updates detail and overlays passed")
	...
	var installed_point := resource.get_point_records()[1]
	assert(is_equal_approx(float(installed_point.get("x", 0.0)), 52.5))
	assert(is_equal_approx(float(installed_point.get("y", 0.0)), -12.0))
	print("✓ PTH point drag install roundtrip passed")
