...
func _initialize() -> void:
	_install_root = ProjectSettings.globalize_path("user://module_designer_pth_connection_remove_test")
	DirAccess.make_dir_recursive_absolute(_install_root.path_join("override"))
	call_deferred("_run_tests")


func _run_tests() -> void:
	await _test_remove_connection_updates_topology()
	await _test_remove_connection_install_roundtrip()
	_cleanup()
	print("✓ Module designer PTH connection remove tests passed")
	quit()


func _test_remove_connection_updates_topology() -> void:
	var editor := await _open_seeded_editor()
	editor._map_view.path_connection_selected.emit(0)
	await process_frame
	editor._remove_selected_path_connection()
	await process_frame

	assert(editor.is_document_dirty())
	assert(editor._path_resource.get_connection_records().size() == 1)
	var remaining := editor._path_resource.get_connection_records()[0]
	assert(int(remaining.get("source_index", -1)) == 1)
	assert(int(remaining.get("target_index", -1)) == 2)
	assert(editor._map_view._selected_path_connection_index == -1)
	print("✓ PTH connection remove updates topology passed")


func _test_remove_connection_install_roundtrip() -> void:
	var editor := await _open_seeded_editor()
	editor._map_view.path_connection_selected.emit(1)
	await process_frame
	editor._remove_selected_path_connection()
	await process_frame
	...
	assert(resource.get_connection_records().size() == 1)
	var installed := resource.get_connection_records()[0]
	assert(int(installed.get("source_index", -1)) == 0)
	assert(int(installed.get("target_index", -1)) == 1)
	print("✓ PTH connection remove install roundtrip passed")
