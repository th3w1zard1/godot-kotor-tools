...
func _initialize() -> void:
	_install_root = ProjectSettings.globalize_path("user://module_designer_pth_export_test")
	DirAccess.make_dir_recursive_absolute(_install_root.path_join("override"))
	call_deferred("_run_tests")


func _run_tests() -> void:
	_test_no_pth_export_fails()
	_test_pth_export_button_wired()
	_test_pth_export_roundtrip()
	_cleanup()
	print("✓ Module designer PTH export tests passed")
	quit()


func _test_no_pth_export_fails() -> void:
	var editor := _build_editor()
	var result := editor.export_pth_preview_to_path(_install_root.path_join("pth_export"))
	assert(not result.get("ok", true))
	print("✓ PTH export without loaded path graph failed as expected")


func _test_pth_export_button_wired() -> void:
	var editor := _build_editor()
	editor.open_resource(_build_git_resource(), "", "tar_m02aa.git")
	assert(editor.pth_file_name() == "tar_m02aa.pth")
	assert(_find_toolbar_button(editor, "Export PTH Preview…") != null)
	print("✓ PTH export toolbar wiring passed")


func _test_pth_export_roundtrip() -> void:
	...
	var editor := _build_editor()
	editor.open_resource(_build_git_resource(), "", "tar_m02aa.git")
	var export_base := _install_root.path_join("exported_path_graph")
	var result := editor.export_pth_preview_to_path(export_base)
	assert(result.get("ok", false))
	var target_path := str(result.get("path", ""))
	assert(target_path.ends_with(".pth"))
	...
	var points := resource.create_document().get_struct_list(resource.get_point_field_name())
	assert(points.size() == 2)
	assert(int(points[0].get("ID", 0)) == 1)
	assert(int(points[1].get("ID", 0)) == 2)
	print("✓ PTH export roundtrip passed")
...
func _cleanup() -> void:
	for path in [
		_install_root.path_join("override").path_join("tar_m02aa.pth"),
		_install_root.path_join("exported_path_graph.pth"),
	]:
