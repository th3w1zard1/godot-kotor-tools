...
func _initialize() -> void:
	_install_root = ProjectSettings.globalize_path("user://module_designer_pth_point_add_test")
	DirAccess.make_dir_recursive_absolute(_install_root.path_join("override"))
	call_deferred("_run_tests")


func _run_tests() -> void:
	await _test_path_point_add_and_install()
	_cleanup()
	print("✓ Module designer PTH point add tests passed")
	quit()


func _test_path_point_add_and_install() -> void:
	...
	assert(editor._path_resource.get_point_count() == 3)
	assert(_find_toolbar_button(editor, "Add Path Point") != null)

	editor._map_view.set_add_path_point_armed(true)
	editor._map_view.path_point_add_requested.emit(12.5, -18.0)
	await process_frame

	assert(not editor._map_view.is_add_path_point_armed())
	assert(editor.is_document_dirty())
	assert(editor._path_resource.get_point_count() == 4)
	assert(editor._map_view._selected_path_point_index == 3)
	assert(editor._viewport_3d._selected_path_point_index == 3)
	assert(editor._detail_label.text.find("Point ID: 4") >= 0)
	assert(editor._detail_label.text.find("Position: 12.50, -18.00, 0.00") >= 0)

	var added_point := editor._path_resource.get_point_records()[3]
	assert(int(added_point.get("id", 0)) == 4)
	assert(is_equal_approx(float(added_point.get("x", 0.0)), 12.5))
	assert(is_equal_approx(float(added_point.get("y", 0.0)), -18.0))
	print("✓ PTH point add updates detail and overlays passed")
	...
	assert(resource.get_point_count() == 4)
	var installed_point := resource.get_point_records()[3]
	assert(int(installed_point.get("id", 0)) == 4)
	assert(is_equal_approx(float(installed_point.get("x", 0.0)), 12.5))
	assert(is_equal_approx(float(installed_point.get("y", 0.0)), -18.0))
	print("✓ PTH point add install roundtrip passed")
