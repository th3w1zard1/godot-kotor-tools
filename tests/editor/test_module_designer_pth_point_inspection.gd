...
func _initialize() -> void:
	_install_root = ProjectSettings.globalize_path("user://module_designer_pth_point_inspection_test")
	DirAccess.make_dir_recursive_absolute(_install_root.path_join("override"))
	call_deferred("_run_tests")


func _run_tests() -> void:
	await _test_path_point_tree_and_selection()
	_cleanup()
	print("✓ Module designer PTH point inspection tests passed")
	quit()


func _test_path_point_tree_and_selection() -> void:
	...
	var path_group := _find_tree_item(editor._instance_tree, "Path Points (3)")
	assert(path_group != null)
	var point_item := _find_tree_item(editor._instance_tree, "Point 2")
	assert(point_item != null)
	point_item.select(0)
	await process_frame

	assert(editor._detail_label.text.find("Point ID: 2") >= 0)
	assert(editor._detail_label.text.find("Outgoing connections: 1") >= 0)
	assert(editor._detail_label.text.find("Targets: 3") >= 0)
	assert(editor._map_view._selected_path_point_index == 1)
	assert(editor._viewport_3d._selected_path_point_index == 1)
	print("✓ PTH point tree selection detail passed")

	editor._map_view.path_point_selected.emit(0)
	await process_frame
	assert(editor._detail_label.text.find("Point ID: 1") >= 0)
	assert(editor._viewport_3d._selected_path_point_index == 0)

	editor._viewport_3d.path_point_selected.emit(2)
	await process_frame
	assert(editor._detail_label.text.find("Point ID: 3") >= 0)
	assert(editor._map_view._selected_path_point_index == 2)

	var path_root := editor._viewport_3d._path_root
	assert(path_root != null)
	assert(editor._viewport_3d._path_point_nodes.has(2))
	print("✓ PTH point map + viewport selection passed")
