...
func _run_tests() -> void:
	...
	if not button_ok:
		...
		quit(1)
	print("✓ WAV batch exporter tests passed")
	quit()
...
func _test_batch_directory_dry_run() -> void:
	...
	assert(str(result.get("summary", "")).contains("Folder batch WAV export"))
	_cleanup(source_root)
	_cleanup(output_root)
	print("✓ WAV folder batch dry-run passed")


func _test_batch_directory_writes_wav() -> void:
	var source_root := _make_dir("source")
	var output_root := _make_dir("output")
	_seed_wavs(source_root)
	...
	_cleanup(source_root)
	_cleanup(output_root)
	print("✓ WAV folder batch write passed")
...
func _test_wav_editor_batch_copy_button() -> bool:
	...
	await process_frame
	assert(_find_button(editor, "Batch Copy WAV Folder...") != null)
	holder.queue_free()
	await process_frame
	print("✓ WAV editor batch copy button passed")
	return true
...
func _make_dir(label: String) -> String:
	...
	var path := ProjectSettings.globalize_path(
		"user://wav_batch_exporter_%s_%d_%d" % [label, _counter, Time.get_ticks_usec()]
	)
