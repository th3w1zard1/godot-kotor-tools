...
func _initialize() -> void:
	_install_root = ProjectSettings.globalize_path("user://module_designer_pth_connection_retarget_test")
	DirAccess.make_dir_recursive_absolute(_install_root.path_join("override"))
	call_deferred("_run_tests")


func _run_tests() -> void:
	await _test_path_connection_retarget_and_install()
	_cleanup()
	print("✓ Module designer PTH connection retarget tests passed")
	quit()


func _test_path_connection_retarget_and_install() -> void:
	...
	var initial_connection := editor._path_resource.get_connection_records()[0]
	assert(int(initial_connection.get("target_index", -1)) == 1)

	editor._select_path_connection(0)
	await process_frame
	editor._map_view.path_connection_retarget_requested.emit(0, 2)
	await process_frame
	...
	var retargeted_connection := editor._path_resource.get_connection_records()[0]
	assert(int(retargeted_connection.get("target_index", -1)) == 2)
	assert(is_equal_approx(float(retargeted_connection.get("target_x", 0.0)), 42.0))
	assert(is_equal_approx(float(retargeted_connection.get("target_y", 0.0)), -28.0))
	print("✓ PTH connection retarget updates detail and overlays passed")
	...
	var installed_connection := resource.get_connection_records()[0]
	assert(int(installed_connection.get("target_index", -1)) == 2)
	print("✓ PTH connection retarget install roundtrip passed")
