...
func _run_tests() -> void:
	...
	_test_batch_install_writes_wav()
	_test_skip_existing()
	var button_ok := await _test_wav_editor_batch_export_button()
	if not button_ok:
		push_error("WAV GameFS batch exporter toolbar test failed")
		quit(1)
	print("✓ WAV GameFS batch exporter tests passed")
	quit()
...
func _test_batch_install_dry_run() -> void:
	...
	var result := WavGamefsBatchExporter.batch_install(gamefs, output_root, {
		"dry_run": true,
	})
	assert(result.get("ok", false))
	var generated: Array = result.get("generated", [])
	assert(generated.size() == 2)
	assert(str(result.get("summary", "")).contains("Install batch WAV export"))
	_cleanup(install_root)
	_cleanup(output_root)
	print("✓ GameFS batch WAV dry-run passed")


func _test_batch_install_writes_wav() -> void:
	var install_root := _make_install_root()
	var output_root := _make_output_root()
	DirAccess.make_dir_recursive_absolute(output_root)
	_seed_install_wavs(install_root)
	...
	var generated: Array = result.get("generated", [])
	assert(generated.size() == 2)
	assert(FileAccess.file_exists(output_root.path_join("line01.wav")))
	assert(FileAccess.file_exists(output_root.path_join("line02.wav")))
	var first: Dictionary = generated[0]
	assert(first.has("channels"))
	assert(first.has("sample_rate"))
	assert(first.has("duration_seconds"))
	assert(first.has("metadata_summary"))
	_cleanup(install_root)
	_cleanup(output_root)
	print("✓ GameFS batch WAV write passed")
...
func _test_skip_existing() -> void:
	...
	DirAccess.make_dir_recursive_absolute(output_root)
	_seed_install_wavs(install_root)
	_write_file(output_root.path_join("line01.wav"), PackedByteArray([0x00]))
	...
	_cleanup(install_root)
	_cleanup(output_root)
	print("✓ GameFS batch WAV skip-existing passed")


func _test_wav_editor_batch_export_button() -> bool:
	var editor := KotorWAVWorkspaceEditor.new()
	var holder := Node.new()
	root.add_child(holder)
	holder.add_child(editor)
	await process_frame
	assert(_find_button(editor, "Batch Export Install WAV...") != null)
	holder.queue_free()
	await process_frame
	print("✓ WAV editor batch export button passed")
	return true
...
func _make_install_root() -> String:
	...
	var install_root := ProjectSettings.globalize_path(
		"user://wav_gamefs_batch_exporter_install_%d_%d" % [_install_counter, Time.get_ticks_usec()]
	)
...
func _make_output_root() -> String:
	...
	return ProjectSettings.globalize_path(
		"user://wav_gamefs_batch_exporter_output_%d_%d" % [_output_counter, Time.get_ticks_usec()]
	)
