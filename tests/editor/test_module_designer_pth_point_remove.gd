...
func _initialize() -> void:
	_install_root = ProjectSettings.globalize_path("user://module_designer_pth_point_remove_test")
	DirAccess.make_dir_recursive_absolute(_install_root.path_join("override"))
	call_deferred("_run_tests")


func _run_tests() -> void:
	await _test_remove_middle_point_clears_incident_edges()
	await _test_remove_leaf_point_preserves_other_edges()
	_cleanup()
	print("✓ Module designer PTH point remove tests passed")
	quit()


func _test_remove_middle_point_clears_incident_edges() -> void:
	var editor := await _open_seeded_editor()
	editor._map_view.path_point_selected.emit(1)
	await process_frame
	editor._remove_selected_path_point()
	await process_frame

	assert(editor.is_document_dirty())
	assert(editor._path_resource.get_point_count() == 2)
	assert(editor._path_resource.get_connection_records().is_empty())
	assert(editor._map_view._selected_path_point_index == -1)
	print("✓ PTH middle point remove clears incident edges passed")


func _test_remove_leaf_point_preserves_other_edges() -> void:
	var editor := await _open_seeded_editor()
	editor._map_view.path_point_selected.emit(2)
	await process_frame
	editor._remove_selected_path_point()
	await process_frame

	assert(editor._path_resource.get_point_count() == 2)
	var connections := editor._path_resource.get_connection_records()
	assert(connections.size() == 1)
	assert(int(connections[0].get("source_index", -1)) == 0)
	assert(int(connections[0].get("target_index", -1)) == 1)
	...
	var installed_connections := resource.get_connection_records()
	assert(installed_connections.size() == 1)
	assert(int(installed_connections[0].get("target_index", -1)) == 1)
	print("✓ PTH leaf point remove install roundtrip passed")


func _open_seeded_editor() -> KotorModuleDesignerWorkspaceEditor:
	var pth_path := _install_root.path_join("override").path_join("tar_m02aa.pth")
